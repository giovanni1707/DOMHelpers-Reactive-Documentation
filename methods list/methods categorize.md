...
### **Query Methods**
- `length` - Get collection length (getter) 
- `first` - Get first item (getter) 
- `last` - Get last item (getter) 
...
- `touchedFields` - Get touched fields array 
- `errorFields` - Get error fields array 
...
- `error` - Property containing error 
...
## **Error Handling**
- `ErrorBoundary` - Error boundary class constructor 
...
- `collector.size` - Get number of cleanup functions 
- `collector.disposed` - Check if collector is disposed 
...
- `hasLocalStorage` - Boolean flag for localStorage availability 
- `hasSessionStorage` - Boolean flag for sessionStorage availability 
...
- `options.storage` - Storage type ('localStorage' or 'sessionStorage') 
- `options.namespace` - Namespace prefix for keys 
- `options.debounce` - Debounce time in milliseconds 
- `options.autoLoad` - Auto-load on creation (default: true) 
- `options.autoSave` - Auto-save on changes (default: true) 
- `options.sync` - Enable cross-tab sync (default: false) 
- `options.expires` - Expiration time in seconds 
- `options.onSave` - Transform before saving callback 
- `options.onLoad` - Transform after loading callback 
- `options.onSync` - Called when synced from another tab 
...
- `options.storage` - Storage type ('localStorage' or 'sessionStorage') 
- `options.namespace` - Namespace prefix for keys 
- `options.immediate` - Call callback immediately with current value 
